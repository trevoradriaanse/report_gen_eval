...
def main():
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to create output directory {args.output_dir}: {str(e)}"
            )
        ...
        if failed_reports:
            ...
            failed_file = (
                ...
                / f"failed_reports_{Path(args.input_file).stem}.jsonl"
            )
            if args.verbose:
        ...
        if results:
            output_file = (
                Path(args.output_dir) / f"results_{Path(args.input_file).stem}.jsonl"
            )
            if args.verbose:
