...
from .utils import (
    ModelProvider, get_model_response, get_text_from_id_fast,
    batch_model_responses, load_jsonl, save_jsonl
)
...
def check_citations_relevance(sentence: str, citation_texts: List[str],
    ...
    responses = batch_model_responses(CHECK_RELEVANCE_SYSTEM, user_prompts, provider, model_name)
    return all(response == "YES" for response in responses)

def check_nugget_matches(sentence: str, nuggets: List[Dict[str, Any]],
                        provider: str = ModelProvider.TOGETHER,
    ...
    responses = batch_model_responses(NUGGET_AGREEMENT_SYSTEM, user_prompts, provider, model_name)
    ...
    for response, nugget_info in zip(responses, nugget_map):
        if response == "YES":
            ...
            if nugget_key not in [(n['question_text'], n['matched_answer']) for n in matched_nuggets]:
                matched_nuggets.append(nugget_info)
    
    return matched_nuggets

def evaluate_sentence(sentence: str, citation_content: Optional[List[str]] = None, 
    ...
    if has_citations:
        ...
        all_citations_relevant = check_citations_relevance(sentence, citation_texts, provider, model_name)
        ...
        if not all_citations_relevant:
            ...
            results["evaluation_details"]["model_responses"].append({
                ...
                "response": "NO",
                "context": {
                    "num_citations": len(citation_texts)
                }
            })
        ...
        if nuggets:
            matched_nuggets = check_nugget_matches(sentence, nuggets, provider, model_name)
            results["matched_nuggets"] = matched_nuggets
            if matched_nuggets:
                results["score"] = len(matched_nuggets)  # Reward for each matched nugget
            else:
        ...
        if is_negative == "YES":
            ...
            if nuggets:
                ...
                results["matched_nuggets"] = matched_nuggets
                if matched_nuggets:
                    results["score"] = 1  # Reward if any nugget confirms negative statement
                else:
                    results["score"] = -1  # Penalize if no nugget supports negative claim
        else:
            ...
            if requires_cite == "YES":
                ...
                if previous_sentences:
                    ...
                    results["evaluation_details"]["is_first_instance"] = is_first == "YES"
                    results["evaluation_details"]["model_responses"].append({
                        "type": "first_instance",
                        "response": is_first,
                        "context": {
                    ...
                    results["score"] = -1 if is_first == "YES" else 0  # Penalize first occurrence, ignore repeats
                else:
        ...
        try:
            ...
            if 'citations' in sentence_data and sentence_data['citations']:
                if isinstance(sentence_data['citations'], list):
                    ...
                    assert len(citation_texts) == len(sentence_data['citations']), f"Expected {len(sentence_data['citations'])} citations, but found {len(citation_texts)}"
                else:
